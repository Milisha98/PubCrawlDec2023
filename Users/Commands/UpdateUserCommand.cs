...
public record UpdateUserCommand(string UserID, string Code, string Name, PokemonEnum Pokemon, bool IsActive) : IRequest<ErrorOr<UserDataModel>>;
...
public class UpdateUserHandler : IRequestHandler<UpdateUserCommand, ErrorOr<UserDataModel>>
{
    ...
    {
        var user = new UserDataModel(request.UserID, request.Code, request.Name, (int)request.Pokemon, request.IsActive);
